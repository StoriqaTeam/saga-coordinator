...
impl NotificationsMicroservice for NotificationsMicroserviceImpl {
    ...
    fn order_create_for_store(&self, initiator: Initiator, payload: OrderCreateForStore) -> ApiFuture<()> {
        ...
        super::request::<_, OrderCreateForStore, ()>(self.http_client.cloned(), Method::Post, url, Some(payload), Some(initiator.into()))
    }

    fn order_create_for_user(&self, initiator: Initiator, payload: OrderCreateForUser) -> ApiFuture<()> {
        let url = format!("{}/users/order-create", self.notifications_url());
        super::request::<_, OrderCreateForUser, ()>(self.http_client.cloned(), Method::Post, url, Some(payload), Some(initiator.into()))
    }
}
